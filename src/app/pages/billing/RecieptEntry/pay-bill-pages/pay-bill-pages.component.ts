...
@Component({
  selector: 'app-pay-bill-pages',
  templateUrl: './pay-bill-pages.component.html',
  styleUrls: ['./pay-bill-pages.component.css']
})
export class PayBillPagesComponent implements OnInit {
   exampleArray = [];
  generateBill$: GenerateBill = new GenerateBill();
  generateBill: GenerateBill = new GenerateBill();
  recieptBill:GenerateBill=new GenerateBill();
  billingHead$: Object;
  occupancy$: Object;
  ...
  constructor(private route: ActivatedRoute, private data: GenerateBillService,private recieptService:RecieptEntryService, private data2: OccupancyService, private data1: BillingHeadService, private router: Router, private formBuilder: FormBuilder) {
  ...
  ngOnInit() {
    ...
    this.registerForm = this.formBuilder.group({
      ...
      recievedAmount:[null, Validators.nullValidator],
      pendingAmount:[null, Validators.nullValidator],
      recDate:[null, Validators.nullValidator],
      year: [null],
    ...
    this.recieptService.getRecieptListByBill(this.generateBill$).subscribe((res:any)=>{
      if(!res.status)
      {
        ...
        if(this.generateBill.pendingAmount>0){
          // this.entries$.push({});
        }
      }
    });	
  ...
  this.recieptBill = new GenerateBill();
